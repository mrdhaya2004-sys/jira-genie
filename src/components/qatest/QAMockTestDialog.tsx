...
const QAMockTestDialog: React.FC<Props> = ({ open, onOpenChange }) => {
  ...
  const loadEntry = async () => {
    ...
    try {
      ...
      console.error(e);
      const msg = e?.message || 'Failed to load challenge';
      setError(msg);
      setStage('entry');
    }
  };
  ...
  const progress = useMemo(() => ((currentIdx + (feedback ? 1 : 0)) / Math.max(1, questions.length)) * 100, [currentIdx, feedback, questions.length]);

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl p-0 overflow-hidden gap-0 max-h-[90vh] flex flex-col">
        <DialogTitle className="sr-only">QA Mock Test — Today's Challenge</DialogTitle>
        <div className="relative bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border-b border-border/60 px-5 py-3 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="h-8 w-8 rounded-lg bg-primary/15 flex items-center justify-center">
              <Brain className="h-4 w-4 text-primary" />
            </div>
            <div>
              <div className="text-sm font-semibold">QA Mock Test</div>
              <div className="text-[11px] text-muted-foreground">Daily Skill Challenge</div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {stage === 'entry' && (
            <div className="p-6 space-y-5">
              <div className="text-center space-y-2">
                ...
                <h2 className="text-xl font-semibold tracking-tight">Sharpen your QA skills</h2>
                ...
                <div className="rounded-lg border border-border/60 p-3 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {stats.weakAreas.map((w) => (
                      <span key={w.category} className="text-[11px] rounded-md bg-amber-500/10 text-amber-700 dark:text-amber-400 px-2 py-0.5">
          ...
          {stage === 'already-done' && completedAttempt && (
            <div className="p-6 space-y-5 text-center">
              <div className="inline-flex h-14 w-14 mx-auto items-center justify-center rounded-full bg-primary/10">
              ...
              {stats && stats.weakAreas.length > 0 && (
                <div className="rounded-lg border border-border/60 p-3 space-y-2 text-left">
                  <div className="flex items-center gap-1.5 text-xs font-medium">
                    <BookOpen className="h-3.5 w-3.5 text-amber-500" />
          ...
          {stage === 'question' && currentQ && (
            <div className="p-5 space-y-4 animate-fade-in">
              ...
              <h3 className="text-base font-medium leading-relaxed">{currentQ.question}</h3>
          ...
          {stage === 'feedback' && currentQ && feedback && (
            <div className="p-5 space-y-4 animate-fade-in">
              ...
              <div className={cn('rounded-lg p-3 text-sm', feedback.correct ? 'bg-green-500/10 border border-green-500/30' : 'bg-amber-500/10 border border-amber-500/30')}>
            ...
            <div className="p-6 space-y-5">
              <div className="text-center space-y-3">
                ...
                {score >= 8 ? (
                  ...
                  <p className="text-sm text-muted-foreground">Good effort! Review the explanations and try again tomorrow.</p>
                )}
              </div>
...
export default QAMockTestDialog;
