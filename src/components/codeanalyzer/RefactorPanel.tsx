...
const VariantView: React.FC<{ variant?: { code: string; changes: string[]; benefits: string[] }; language: string }> = ({ variant, language }) => {
  ...
  if (!variant || !variant.code) return <div className="text-sm text-muted-foreground p-6 text-center border border-dashed rounded-lg">No significant optimization opportunity detected. The code already follows acceptable practices for readability, stability, and structure at this level.</div>;
