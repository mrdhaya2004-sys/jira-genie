    ...
    <Dialog open onOpenChange={(o) => { if (!o) onCancel(); }}>
      <DialogContent className="sm:max-w-[480px] p-0 overflow-hidden">
        <div className="bg-gradient-to-br from-orange-500/10 via-orange-400/5 to-transparent px-7 pt-7 pb-5 text-center">
          ...
          <DialogTitle className="text-lg font-semibold">Connect GitLab</DialogTitle>
        ...
        <form onSubmit={onSubmit} className="px-7 pb-7 space-y-4">
          <div className="space-y-1.5">
            ...
            <Input id="gl-url" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder="https://gitlab.com" required />
            <p className="text-[11px] text-muted-foreground">For self-hosted, use your full base URL.</p>
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="gl-token">Personal Access Token</Label>
            <Input id="gl-token" type="password" value={token} onChange={(e) => setToken(e.target.value)} placeholder="glpat-…" required />
            <a
              href="https://gitlab.com/-/user_settings/personal_access_tokens"
              target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-1 text-[11px] text-primary hover:underline"
            >
              Create a token with <code className="rounded bg-muted px-1">api</code> scope
